(ns com.outskirtslabs.vlc.bus
  (:require
   [clojure.core.async :as async]
   [com.outskirtslabs.vlc.commands :as cmd]
   [com.outskirtslabs.vlc.interop.player :as player]
   [com.outskirtslabs.vlc.util :as util]))

(defn ensure-not-released! [{:com.outskirtslabs.vlc.impl/keys [state_]
                             :as i}]
...
(defn dispatch-control-event [instance e]
  ...
  (if (cmd/native? e)
    (player/run-on-player-thread (:com.outskirtslabs.vlc.impl/player instance)
                                 (fn []
                                   (try
                                     (ensure-not-released! instance)
...
(defn start-control-loop! [{:com.outskirtslabs.vlc.impl/keys [<control <close] :as instance}]
  (util/thread
    (let [halt* (promise)]
      (loop []
        (try
...
(defn start-event-loop! [{:com.outskirtslabs.vlc.impl/keys [player <close state_] :as _instance}]
...
(defn dispatch-event!
  ...
  [{:com.outskirtslabs.vlc.impl/keys [<control] :as i} event]
...
(defn subscribe-impl!
  [{:com.outskirtslabs.vlc.impl/keys [state_]} event-pred callback-fn]
...
(defn unsubscribe-impl!
  [{:com.outskirtslabs.vlc.impl/keys [state_]} subscription-id]
