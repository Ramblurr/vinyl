(ns com.outskirtslabs.vlc.playback
  (:require
   [com.outskirtslabs.vlc.interop.api :as api]
   [com.outskirtslabs.vlc.bus :as bus]
   [com.outskirtslabs.vlc.commands :as cmd]
   [com.outskirtslabs.vlc.interop.parsing :as parsing]
   [com.outskirtslabs.vlc.queue :as queue]))

(defn handle-player-event [{:com.outskirtslabs.vlc.impl/keys [state_] :as instance} event]
  (when-not (contains? #{:vlc/time-changed :vlc/position-changed} (:event event))
    (tap> event)))

(defn init! [{:com.outskirtslabs.vlc.impl/keys [state_] :as instance}]
...
(defn- queue [{:com.outskirtslabs.vlc.impl/keys [state_] :as _instance}]
  (get-in @state_ [:playback :queue]))

(defn- set-queue [{:com.outskirtslabs.vlc.impl/keys [state_] :as _instance} new-queue]
  (tap> [:set-queue new-queue])
  (swap! state_ assoc-in [:playback :queue] new-queue))

(defn release! [{:com.outskirtslabs.vlc.impl/keys [state_] :as instance}]
...
(defn expand-paths [instance tracks]
  (let [result @(parsing/parse-meta tracks {:media-player-factory (get-in instance [:com.outskirtslabs.vlc.impl/player :vlc/media-player-factory])})]
...
(defmethod cmd/dispatch :playback/play
  ...
  (let [queue (queue instance)
        ...
        state (api/get-state (:com.outskirtslabs.vlc.impl/player instance))
