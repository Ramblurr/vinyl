(ns com.outskirtslabs.vlc.interop.api
  (:require [com.outskirtslabs.vlc.commands :as cmd]
            [com.outskirtslabs.vlc.interop.enum :as enum])
  (:import
...
(defmethod cmd/dispatch :vlcj.media-api/play
  [{{:vlc/keys [^MediaPlayer media-player]} :com.outskirtslabs.vlc.impl/player :as instance} {:keys [mrl-or-media-ref options] :as e}]
  (cmd/ensure-valid! e)
...
(defmethod cmd/dispatch :vlcj.media-api/reset
  [{{:vlc/keys [^MediaPlayer media-player]} :com.outskirtslabs.vlc.impl/player} e]
  (cmd/ensure-valid! e)
  (let [^MediaApi media-api (.media media-player)]
    (.reset media-api)))

(defmethod cmd/dispatch :vlcj.media-api/prepare
  [{{:vlc/keys [^MediaPlayer media-player]} :com.outskirtslabs.vlc.impl/player} {:keys [mrl-or-media-ref options] :as e}]
  (cmd/ensure-valid! e)
  (let [^MediaApi media-api (.media media-player)
        ^String/1 options (when options (into-array String options))]
    (if (string? mrl-or-media-ref)
